...
function Horizontal({image, cat, desc, alt, title, onClick, setRef, isActive}) {
    return (
    <div style={{cursor:`${isActive?'':'pointer'}`}} onClick={isActive ? null : onClick} ref={setRef} className={`${isActive? styles.active : ''} ${styles.horizontalType} ${styles.postBlog}`}>
        {!isActive ? 
