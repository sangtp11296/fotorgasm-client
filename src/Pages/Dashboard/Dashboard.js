...
function Dashboard() {
    ...
    const [popupAdmin, setPopupAdmin] = useState([false, false, false, false]);
    ...
    // Toggle Popup Admin
    const isAllTrue = popupAdmin.every((element) => element === false);
    function togglePopupAdmin(){
        if (isAllTrue) {
            setPopupAdmin([true, true, true, true]);
        }
        else {
            setPopupAdmin([false, false, false, false]);
        }
    }
    // Handle Image Upload
            ...
            <div className={`${styles.headerContainer} ${styles.gridBlock}`}>
                <div className={styles.firstSection}>
                    ...
                    <div className={`${styles.editPart} ${styles.bubble}`} >
                        <div style={{height:'38px', width:'38px'}} className={styles.iconContainer} onClick={togglePopupAdmin}>
                            {
                                !isAllTrue ?
                    ...
                    <form id='adminUpdate' className={styles.editForm} onSubmit={handleAdminUpdate}>
                        <label htmlFor='avatarUpload' style={{transform: popupAdmin[0] ? 'translateX(10px)' : 'translateX(0px)', opacity: popupAdmin[0] ? '1' : '0', visibility: popupAdmin[0] ? 'visible' : 'hidden'}} className={`${styles.iconContainer} ${styles.editItem}`}>
                            <input hidden id='avatarUpload' type="file" accept="image/*" onChange={handleImageSelected} />
                        ...
                        <label htmlFor='adminNameUpdate' 
                            style={{
                                transform: popupAdmin[1] && popupAdmin[0] ? 'translateX(20px)' : popupAdmin[1] && !popupAdmin[0] ? 'translateX(-5px)' : 'translateX(0px)',
                                opacity: popupAdmin[1] ? '1' : '0',
                                visibility: popupAdmin[1] ? 'visible' : 'hidden'
                            }} 
                            className={`${styles.iconContainer} ${styles.editItem}`}
                            onClick={() => setPopupAdmin([false, true, false, false])}>
                            ...
                            <input placeholder='fotorgasm' type={popupAdmin[1] && !popupAdmin[0] ? 'text' : 'hidden'} id='adminNameUpdate' onChange={(e)=>setNewName(e.target.value)} />
                        </label>
                        <label htmlFor='adminUpdateTitle' style={{
                                transform: popupAdmin[2] && popupAdmin[0] ? 'translateX(30px)' : popupAdmin[2] && !popupAdmin[0] ? 'translateX(-25px)' : 'translateX(0px)',
                                opacity: popupAdmin[2] ? '1' : '0',
                                visibility: popupAdmin[2] ? 'visible' : 'hidden'
                            }}
                            className={`${styles.iconContainer} ${styles.editItem}`}
                            onClick={() => setPopupAdmin([false, false, true, false])}>
                            ...
                            <input placeholder='Project Admin' type={popupAdmin[2] && !popupAdmin[0] ? 'text' : 'hidden'} id='adminUpdateTitle' onChange={(e)=>setNewName(e.target.value)} />
                        </label>
                        <label htmlFor='adminUpdatePassword' style={{
                                transform: popupAdmin[3] && popupAdmin[0] ? 'translateX(40px)' : popupAdmin[3] && !popupAdmin[0] ? 'translateX(-45px)' : 'translateX(0px)',
                                opacity: popupAdmin[3] ? '1' : '0',
                                visibility: popupAdmin[3] ? 'visible' : 'hidden'
                            }}
                            className={`${styles.iconContainer} ${styles.editItem}`}
                            onClick={() => setPopupAdmin([false, false, false, true])}>
                            ...
                            <input placeholder='Project Admin' type={popupAdmin[3] && !popupAdmin[0] ? 'text' : 'hidden'} id='adminUpdatePassword' onChange={(e)=>setNewName(e.target.value)} />
                        </label>
                    </form>
                </div>
