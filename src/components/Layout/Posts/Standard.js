...
const Standard = (({image, alt, title, cat, desc, onClick, setRef, isActive, isUnActive}) => {
  return (
    <div style={{cursor:`${isActive?'':'pointer'}`}} onClick={isActive ? null : onClick} ref={setRef} className={`${isActive? styles.active : ''} ${styles.standardType} ${styles.postBlog}`}>
        {!isActive ? 
            ...
            <button className={styles.minorScreen} onClick={onClick}>
